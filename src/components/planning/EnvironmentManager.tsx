...
import { getEnvironments, addEnvironment, updateEnvironment, deleteEnvironment } from '@/config/firebase';
...
export function EnvironmentManager({ instituteId, onDataChange }: EnvironmentManagerProps) {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const fetchedData = await getEnvironments(instituteId);
  ...
  const onSubmit = async (data: EnvironmentFormValues) => {
    try {
        if (selectedEnvironment) {
            await updateEnvironment(instituteId, selectedEnvironment.id, data);
            toast({ title: "Ambiente Actualizado", description: "El ambiente ha sido actualizado correctamente." });
        } else {
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
        await deleteEnvironment(instituteId, selectedEnvironment.id);
            ...
            {paginatedData.length > 0 ? paginatedData.map((env) => (
              <TableRow key={env.id}>
                ...
                <TableCell className="text-right">
                 ...
                 <TableRow><TableCell colSpan={5} className="h-24 text-center">No hay ambientes registrados.</TableCell></TableRow>
            )}
          </TableBody>
        </Table>
      </div>
       ...
       <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
            <DialogContent>
                <DialogHeader>
                    ...
                    <DialogDescription>Completa los detalles del espacio físico.</DialogDescription>
                </DialogHeader>
                <Form {...form}>
