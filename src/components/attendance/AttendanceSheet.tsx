...
export function AttendanceSheet({ students, attendanceRecord, totalWeeks, onAttendanceChange }: AttendanceSheetProps) {
    ...
    return (
        <Accordion type="single" collapsible className="w-full space-y-4" value={activeWeek} onValueChange={setActiveWeek}>
            {Array.from({ length: totalWeeks }, (_, i) => i + 1).map(weekNumber => (
                <AccordionItem key={weekNumber} value={`week-${weekNumber}`} className="border rounded-lg shadow-sm">
                    <AccordionTrigger className="text-lg font-medium px-6 py-4">
                        Semana {weekNumber}
                    </AccordionTrigger>
                    <AccordionContent>
                        <div className="relative w-full overflow-auto rounded-b-lg border-t">
                             <Table>
                                <TableHeader>
                                    <TableRow>
                                            ...
                                            <TableRow key={student.documentId}>
                                                ...
                                                {Array.from({ length: 5 }).map((_, dayIndex) => (
                                                    <TableCell key={dayIndex} className="text-center p-1">
                                                        ...
                                                        >
                                                            <SelectTrigger className={cn("w-full border-0 focus:ring-0", getStatusColor(weeklyData[dayIndex]))}>
