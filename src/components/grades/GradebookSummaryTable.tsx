                    ...
                    <Table className="border-separate border-spacing-0 table-auto w-full print:border-collapse">
                        <TableHeader className="sticky top-0 z-50 print:relative print:z-0">
                            <TableRow className="bg-slate-100 hover:bg-slate-100 print:bg-gray-100">
                                ...
                                <TableHead className="w-[220px] sticky left-[40px] top-0 bg-slate-100 z-[60] font-bold text-[10px] uppercase border-r border-b shadow-[2px_0_5px_rgba(0,0,0,0.1)] print:relative print:border-black print:shadow-none print:w-[150px]">Apellidos y Nombres</TableHead>
                                {indicators.map(indicator => (
                                    <TableHead key={indicator.id} className="text-center p-2 min-w-[70px] print:min-w-[30px] border-r border-b bg-slate-50 print:relative print:border-black print:bg-gray-50 grade-col">
                                        <div className="flex flex-col items-center">
                                            <span className="text-[8px] uppercase text-muted-foreground font-black tracking-tight print:text-black">Ind.</span>
                                            <span className="text-[10px] font-bold leading-tight line-clamp-1 max-w-[70px] print:max-w-[40px] text-primary print:text-black">{indicator.name}</span>
                                        </div>
                                    </TableHead>
                                ))}
                                <TableHead className="text-center w-[80px] print:w-[50px] sticky right-0 top-0 bg-primary/10 text-primary z-[60] font-black text-[10px] uppercase border-l border-b shadow-[-2px_0_5px_rgba(0,0,0,0.1)] print:relative print:border-black print:text-black print:shadow-none grade-col">PROM. FINAL</TableHead>
                            </TableRow>
                        </TableHeader>
                        <TableBody>
                                    ...
                                    <TableRow key={student.documentId} className="h-10 hover:bg-slate-50 transition-colors print:h-8 print:bg-white">
                                        ...
                                        {indicatorAverages.map((avg, i) => (
                                            <TableCell key={indicators[i].id} className={cn(
                                                "text-center font-bold text-[10px] border-r border-b print:border-black grade-col",
                                        ...
                                        <TableCell className={cn(
                                            "sticky right-0 bg-primary/5 z-10 text-center font-black text-xs border-l border-b shadow-[-2px_0_5px_rgba(0,0,0,0.05)] print:relative print:border-black print:bg-transparent print:shadow-none grade-col",
