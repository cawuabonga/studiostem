...
interface PrintSupplyRequestProps {
    request: SupplyRequest;
    institute: Institute | null;
}

export function PrintSupplyRequest({ request, institute }: PrintSupplyRequestProps) {
        ...
        <div className="print-page-preview p-8 font-sans text-black bg-white">
             <header className="print-header flex items-center justify-between mb-4 pb-4 border-b-2 border-black">
                ...
                <div className="border-2 border-black p-2 text-center">
                    <h2 className="font-bold text-lg">SOLICITUD N°</h2>
                    <p className="font-mono text-base">{request.code || 'S/C'}</p>
                </div>
            </header>

            <div className="text-center my-6">
                <h2 className="text-2xl font-bold uppercase">CARGO DE SOLICITUD DE INSUMOS</h2>
            </div>
            ...
            <table className="w-full text-xs border-collapse border border-black">
                 <thead className="bg-gray-100">
                    <tr>
                        ...
                        <th className="border p-2 text-center">CANTIDAD SOLICITADA</th>
                    </tr>
                </thead>
                 <tbody>
                    {request.items.map((item, index) => (
                        <tr key={item.itemId}>
                            ...
                            <td className="border p-1 text-center">{item.requestedQuantity}</td>
                        </tr>
                    ))}
             ...
             <footer className="text-center mt-32 grid grid-cols-2 gap-8">
                <div className="inline-block border-t border-black px-8 py-2 mx-auto">
                    ...
                    <p className="text-xs mt-1">Nombre y DNI</p>
                </div>
                 <div className="inline-block border-t border-black px-8 py-2 mx-auto">
                    <p>Recibido por Almacén</p>
