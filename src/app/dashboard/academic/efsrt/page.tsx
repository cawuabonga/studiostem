...
import { MapPin, Calendar, Clock, CheckCircle, MessageSquare, Loader2, Users } from 'lucide-react';
...
export default function StudentEFSRTPage() {
    ...
    const handleUploadReport = async (assignmentId: string, event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            toast({ title: "Informe Subido", description: "Enviado para evaluación." });
            fetchData();
        } catch (error) {
        ...
        <div className="space-y-6">
            <Card className="bg-primary text-primary-foreground">
                <CardHeader>
                    ...
                    <CardDescription className="text-primary-foreground/80">Seguimiento automático basado en calendario.</CardDescription>
                </CardHeader>
            </Card>
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                {program?.modules.map(module => {
                    ...
                    return (
                        <Card key={module.code} className={`flex flex-col border-t-4 ${effectiveStatus === 'Aprobado' ? 'border-t-green-500' : 'border-t-primary'}`}>
                            <CardHeader>
                            ...
                            <CardContent className="flex-grow space-y-4">
                                {assignment ? (
                                    <div className="space-y-3 text-sm">
                                        <div className="flex items-center gap-2 text-muted-foreground"><MapPin className="h-4 w-4 text-primary" /><span className="font-medium text-foreground">{assignment.location}</span></div>
                                        ...
                                        <div className="flex items-center gap-2 p-2 bg-muted rounded-md"><Users className="h-4 w-4 text-primary" /><div className="flex flex-col"><span className="text-[10px] text-muted-foreground">Supervisor</span><span className="font-bold text-xs">{assignment.supervisorName}</span></div></div>
                                        {assignment.visits.length > 0 && (
                                            <div className="pt-2">
                                                <p className="font-bold text-xs mb-2 flex items-center gap-1"><MessageSquare className="h-3 w-3 text-primary" /> Visitas ({assignment.visits.length})</p>
                                                <div className="space-y-2 max-h-32 overflow-y-auto pr-1">
                                                    {assignment.visits.map((visit, i) => (
                                                        <div key={i} className="bg-muted/50 p-2 rounded text-[10px] border">
                                                            <div className="flex justify-between font-bold mb-1 opacity-70"><span>{format(visit.date.toDate(), 'dd/MM/yy')}</span><span>{visit.type}</span></div>
                                    ...
                                    <div className="flex flex-col items-center justify-center py-10 text-center space-y-2 opacity-50"><Clock className="h-8 w-8" /><p className="text-xs italic">Aún no se ha programado para este módulo.</p></div>
                                )}
                            </CardContent>
                            <CardFooter className="pt-0">
                                {assignment && effectiveStatus !== 'Aprobado' && effectiveStatus !== 'Desaprobado' && (
                                    <div className="w-full space-y-2">
                                        ...
                                        <div className="flex gap-2">
                                            <Input id={`file-${assignment.id}`} type="file" onChange={(e) => handleUploadReport(assignment.id, e)} disabled={!!isUploading} className="h-8 text-[10px]" />
                                ...
                                {effectiveStatus === 'Aprobado' && (
                                    <div className="w-full bg-green-50 border border-green-100 p-2 rounded-md text-center">
                                        ...
                                        <p className="text-green-600 text-[10px] font-medium flex items-center justify-center gap-1"><CheckCircle className="h-3 w-3" /> MÓDULO CULMINADO</p>
                                    </div>
                                )}
                            </CardFooter>
                        </Card>
